...
const selectedProjects = {
  trilogy: {
    description: (
      <p>
        Wrapper for SQLite databases in JavaScript, written in TypeScript.
        It is designed to make using SQL queries more ergonomic within
        JavaScript code.
      </p>
    )
  },
  ...
  cascade: {
    description: (
      <p>
        Method, accessor, and assignment cascades for Nim implemented as
        a compile-time macro, inspired by Smalltalk &amp; Dart.
      </p>
    )
  },
  glob: {
    description: (
      <p>
        Pure Nim library for searching for files that match Unix-style &ldquo;glob&rdquo;
        patterns, such as <code>./assets/**/*.png</code>.
      </p>
    )
  },
  fugitive: {
...
export const getServerSideProps: GetServerSideProps = async _context => {
  ...
  if (token == null || token === "") {
    console.error("Projects: no GitHub token found in WEBSITE_GITHUB_TOKEN environment variable.")
    return {
      props: {
        repos: []
      }
...
query {
  ...
  try {
    ...
    console.error("Failed to retrieve repository information from GitHub")
  }
  ...
  return (
    <section className={styles.projectListing}>
      <h3 className={styles.projectListingHeader}>
        ...
        {repo.homepageUrl !== "" && <IconLink href={repo.homepageUrl} Icon={Globe} title="Homepage" size="14" />}
        {repo.url !== "" && <IconLink href={repo.url} Icon={Code} title="Source" size="14" />}
        {
          repo.stargazerCount > 0 && (
            <a href={repo.url + "/stargazers"} title="GitHub Stars" className={styles.projectListingStarCount}>
      ...
      <Content header="projects">
...

        {
          repos.length > 0 && repos.map(repo =>
            <ProjectListing repo={repo} key={repo.name} />
          )
        }
      </Content>
    </Main>
